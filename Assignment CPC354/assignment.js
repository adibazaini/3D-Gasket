...
window.onload = function init() {
  ...
  startBtn.addEventListener("click", () => {
    if (!gasket.pause) {
      ...
      startBtn.style.background = "#BBE6AD";
    } else {
      gasket.pause = false;
      ...
      startBtn.value = "Stop";
      startBtn.style.background = "#FF7F50";
    }
  });
...
function rotation(obj, degree, axis) {
  ...
  if (obj.rotateXYZ[axis] === true) {
    ...
    if (Math.abs(difference) > obj.speed) {
      // add/subtract based on sign
      obj.theta[axis] += Math.sign(difference) * obj.speed;
      return false;
    } else {
...
function scaling(obj, scaleFac) {
  ...
  if (Math.abs(difference) > obj.speed * 0.05) {
    // add/subtract based on sign
    obj.scale += Math.sign(difference) * obj.speed * 0.05;
    return false;
  } else {
    obj.scale = scaleFac;
    return true;
